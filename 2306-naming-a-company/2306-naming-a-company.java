        ...
        for (int i = 0; i < 25; ++i) {
            for (int j = i + 1; j < 26; ++j) {
                ...
                for (String ideaA : initialGroup[i]) {
                    if (initialGroup[j].contains(ideaA)) { //these line checks if set1's elements are contained by set2 or not. if so then increase value of mutual and then in last decrease by total size of the set
                        numOfMutual++;
                    }
                }

                // Valid names are only from distinct suffixes in both groups.
                answer += (initialGroup[i].size() - numOfMutual) * (initialGroup[j].size() - numOfMutual);
            }
        }
